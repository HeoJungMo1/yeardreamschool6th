...
SELECT productName
    , buyPrice
    ,ROW_NUMBER() OVER(ORDER BY buyPrice) AS row_num
FROM products
...
SELECT customerNumber
    , customerName
    , totalPayment
    , RANK() OVER(ORDER BY totalPayment) AS rnk
    , DENSE_RANK() OVER(ORDER BY totalPayment) AS dense_rnk
    , ROW_NUMBER() OVER(ORDER BY totalPayment) AS row_num
FROM (
...
FROM (
    SELECT c.customerNumber
        , c.customerName
        , strftime("%Y-%m", p.paymentDate) AS payYearMonth
        , ROUND(AVG(p.amount), 2) AS avgMonthAmount
        , RANK() OVER (
                PARTITION BY strftime("%Y-%m", p.paymentDate)
                ORDER BY SUM(p.amount) DESC) AS monthAvgRank
    FROM customers c
    JOIN payments p
...
SELECT customerNumber
    , paymentDate
    , amount
    , SUM(amount) OVER (ORDER BY paymentDate
...
SELECT customerNumber
    , paymentDate
    , amount
    , LAG(amount, 1) OVER (PARTITION BY customerNumber
                            ORDER BY paymentDate) AS prevAmount
FROM payments
...
SELECT customerNumber
    ...
    , LEAD(amount, 1) OVER (PARTITION BY customerNumber
                            ORDER BY paymentDate) AS nextAmount
FROM payments
LIMIT 15;
...
SELECT DISTINCT country
    , FIRST_VALUE(customerName) OVER (PARTITION BY country
                                    ORDER BY paymentDate DESC) AS firstCustomer
...
SELECT paymentYear AS year
    , SUM(totalPayment) AS totalPayment_2nd_quartile
FROM (
    SELECT c.customerNumber
        , c.customerName
        , strftime('%Y', p.paymentDate) AS paymentYear
        , SUM(p.amount) AS totalPayment
        , NTILE(4) OVER (PARTITION BY strftime('%Y', p.paymentDate)
                        ORDER BY SUM(p.amount) DESC) AS paymentQuartile
    FROM customers c
    JOIN payments p
        ON c.customerNumber = p.customerNumber
    GROUP BY paymentYear, c.customerNumber
) AS sub
WHERE paymentQuartile = 2
