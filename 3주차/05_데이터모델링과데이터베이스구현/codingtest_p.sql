...
/* ----------------------------------------------------------------
   ...
   ---------------------------------------------------------------- */
SELECT c.country, ROUND(SUM(quantityOrdered * priceEach), 2) AS total_revenue
FROM customers c
JOIN orders o
...
SELECT c.customerNumber
    , c.customerName
    , COUNT(DISTINCT o.orderNumber) AS purchase_count
FROM customers c
JOIN orders o
    ON c.customerNumber = o.customerNumber
JOIN orderdetails od
...
/* ----------------------------------------------------------------
   ...
   -- 테이블명: customers
   ---------------------------------------------------------------- */
SELECT c1.customerNumber, c1.customerName, c1.creditLimit
FROM customers c1
WHERE creditLimit > (
...
FROM customers c1
JOIN (
   SELECT AVG(creditLimit) AS avg_cl
   FROM customers
) AS c2
   ON c1.creditLimit > c2.avg_cl
ORDER BY c1.creditLimit DESC;
...
JOIN (
   ...
   FROM orders
   WHERE strftime("%Y", orderDate) = '2003'
) AS o_2003 ON c.customerNumber = o_2003.customerNumber
JOIN (
   SELECT
   FROM orders
   WHERE strftime("%Y", orderDate) = '2004'
) AS o_2004 ON c.customerNumber = o_2004.customerNumber
...
/* ----------------------------------------------------------------
   ...
   -- 테이블명: products, orderdetails
   ---------------------------------------------------------------- */
SELECT productLine, line_revenue
FROM (
   SELECT p.productLine
...
-- 인라인 뷰
SELECT customerNumber, customerName, total_payment, payment_rank
FROM (
   SELECT c.customerNumber
   , c.customerName
   , SUM(p.amount) AS total_payment
...
FROM (
   SELECT strftime("%Y-%m", o.orderDate) AS year_month
      ...
      , LAG(ROUND(SUM(od.quantityOrdered * od.priceEach), 2)) 
         OVER (ORDER BY strftime("%Y-%m", o.orderDate)) AS prev_month_revenue
   FROM orders o
   JOIN orderdetails od
      ON o.orderNumber = od.orderNumber
...
SELECT productLine
   , line_revenue
   , ROUND((line_revenue / SUM(line_revenue) OVER ()) * 100, 2) AS revenue_ratio
FROM (
   SELECT p.productLine
   , ROUND(SUM(quantityOrdered * priceEach), 2) AS line_revenue
   FROM products p
   JOIN orderdetails pl
