...
SELECT
    c.CustomerId
    , c.FirstName || ' ' || c.LastName AS CustomerNmae
    , c.Country
...
SELECT
    ...
    i.BillingCountry,
    i.Total,
    AVG(Total) OVER (PARTITION BY BillingCountry) AS CounrtyAvgTotal
FROM invoices
...
SELECT
    ...
    , SUM(Total) / (SELECT SUM(Total) FROM invoices)
FROM invoices
GROUP BY BillingCountry
...
SELECT
    ...
    , Title
    , HireDate
    , NTILE(3) OVER (ORDER BY HireDate)
FROM employees e
ORDER BY HireDate
...
SELECT
    g.Name              AS Genre
    , mt.Name           AS MdeiaType
    , COUNT(t.TrackId)  AS TrackCount -- 장르와 미디어타입별로 트랙의 수 (TrackCount)
    , ROUND(AVG(t.UnitPrice), 2) AS AvgPirce -- 장르와 미디어타입별로 평균 단가 (AvgPirce)
FROM tracks t
