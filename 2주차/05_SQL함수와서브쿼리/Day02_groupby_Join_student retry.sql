/* =====================================================================
   4장 다수의 테이블 제어하기 — 실습 문제 20제 (수강생용)
   - 강의자료 순서 준수:
...
SELECT ArtistId, COUNT(*) AS '아티스트별 앨범 수'
FROM albums
GROUP BY ArtistId;
...
SELECT BillingCountry, SUM(Total) AS '국가별매출합계'
FROM invoices
...
SELECT GenreId, ROUND(AVG(Milliseconds), 2) AS '장르별 평균 재생시간'
FROM tracks
GROUP BY GenreId
...
SELECT CustomerId, COUNT(*) AS ivoice_cnt
FROM invoices
GROUP BY CustomerId
HAVING COUNT(*) >= 7;
...
SELECT ii.InvoiceLineId, t.Name, ii.UnitPrice, ii.Quantity
